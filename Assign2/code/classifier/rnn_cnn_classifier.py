...
'''
class RNNModelCNN(nn.Module):
    def __init__(self, rnn_type, ntoken, ninp, nhid, nlayers, batchsize, bptt):
        super(RNNModelCNN, self).__init__()
        self.encoder = nn.Embedding(ntoken, ninp)
        self.conv1 = nn.Conv1d(bptt,bptt, 3, stride = 1)
        self.maxpool =F.max_pool1d
	self.rnn = getattr(nn, rnn_type)(149, nhid, nlayers, bias=False)
