...
class RNNModelGlove(nn.Module):
    """Container module with an encoder, a recurrent module, and a decoder."""

    def __init__(self, rnn_type, ntoken, ninp, nhid, nlayers, dropout=0.5, tie_weights=False,  embeddingWeights = None):
        super(RNNModelGlove, self).__init__()
        self.encoder = nn.Embedding(ntoken, ninp)
        self.rnn = getattr(nn, rnn_type)(ninp, nhid, nlayers, bias=False, dropout=dropout)
        self.decoder = nn.Linear(nhid, ntoken)
        ...
        self.init_weights(embeddingWeights)
