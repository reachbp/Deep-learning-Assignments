...
#        print("c size",c.size())
  ...
  #      print("Mat1 size", mat1.size())
        ...
        h = F.leaky_relu(inputs @ Wzh.cuda() + mat2)
        X = F.tanh(torch.mm(h , Whx.cuda())  + bhx.repeat(h.size(0), 1).cuda())
...
class _NetDC(nn.Module):
    ...
    def forward(self, z, c):
        ...
        inputs = torch.cat([z, c], 1).cuda()
        h = F.leaky_relu(inputs @ Wxh.cuda() + bxh.repeat(inputs.size(0), 1).cuda())
...
for epoch in range(opt.niter):
 ...
 #       print(i)
        ...
        if Dis_loss > 1.0:
            ...
            print("D_loss_real", D_loss_real.data.cpu().numpy())
            print("D_loss_fake", D_loss_fake.data.cpu().numpy())
            D_loss.backward()
            optimizerD.step()
            Dis_loss = D_loss.data.cpu().numpy()
            print("Discrimiantor Loss", Dis_loss)
